...
"Ammo_Marker" setMarkerText "Abandoned Repair Truck";
"Ammo_Marker" setMarkerColor "ColorRed";

mtvrrepair = createVehicle ["MtvrRepair",[(getpos _hill select 0), getpos _hill select 1,0],[], 0, "NONE"];

if(isnull mtvrrepair) exitwith 
{
...
_color = "#C5C5C5";
_name = getText (configFile >> "cfgVehicles" >> typeOf mtvrrepair >> "displayName");
_picture = getText (configFile >> "cfgVehicles" >> typeOf mtvrrepair >> "picture");
...
{ 
    ...
    (_temptime >= _missionTimeOut) or (PlayerPresent == 1) or getpos mtvrrepair distance getMarkerPos _marker > _missionTriggerRadius
};

if(_temptime >= _missionTimeOut) then
{
    mtvrrepair setDamage 1;
