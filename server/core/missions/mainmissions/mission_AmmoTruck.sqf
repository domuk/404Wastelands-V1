...
"Ammo_Marker" setMarkerText "Abandoned Ammo Truck";
"Ammo_Marker" setMarkerColor "ColorRed";

mtvrammo = createVehicle ["MtvrReammo",[(getpos _hill select 0), getpos _hill select 1,0],[], 0, "NONE"];

if(isnull mtvrammo) exitwith 
{
...
_color = "#C5C5C5";
_name = getText (configFile >> "cfgVehicles" >> typeOf mtvrammo >> "displayName");
_picture = getText (configFile >> "cfgVehicles" >> typeOf mtvrammo >> "picture");
...
{ 
    ...
    (_temptime >= _missionTimeOut) or (PlayerPresent == 1) or getpos mtvrammo distance getMarkerPos _marker > _missionTriggerRadius
};

if(_temptime >= _missionTimeOut) then
{
    mtvrammo setDamage 1;
